...
public class SetAvailabilityActivity extends ParentActivity implements DaySlotAdapter.setRecentDateSlotClickList, TimeSlotAdapter.setRecentTimeSlotClickList {
    ...
    private void setDayData() {
        ...
        if (generalFunc.retrieveValue(Utils.LANGUAGE_CODE_KEY).equalsIgnoreCase("th")) {
            ...
            namesOfDays1 = DateFormatSymbols.getInstance(MyUtils.getLocale()).getShortWeekdays();
        } else {
            namesOfDays = DateFormatSymbols.getInstance(Locale.ENGLISH).getWeekdays();
            namesOfDays1 = DateFormatSymbols.getInstance(MyUtils.getLocale()).getWeekdays();
        }
        ...
        if (generalFunc.retrieveValue(Utils.LANGUAGE_CODE_KEY).equalsIgnoreCase("th")) {
            passnamesOfDays = DateFormatSymbols.getInstance(MyUtils.getLocale()).getShortWeekdays();
        } else {
            passnamesOfDays = DateFormatSymbols.getInstance(MyUtils.getLocale()).getWeekdays();
        }
    ...
    private void addTimeSlotApi() {
       ...
       /* if (vAvailableTime.equalsIgnoreCase(selectedtime)) {
            if (timeadapter != null && timeadapter.isAllItemsChecked()) {
                generalFunc.showMessage(titleTxt, generalFunc.retrieveLangLBl("", "LBL_ALL_SLOT_SELECTED"));
            } else {
                generalFunc.showMessage(titleTxt, generalFunc.retrieveLangLBl("", "LBL_WHAT_TIME"));
            }
            return;
        }*/
        vAvailableTime = selectedtime;
