...
if ($sortby == 1) {
    if ($order == 0) $ord = " ORDER BY vCountry ASC"; else
        $ord = " ORDER BY vCountry DESC";
}
if ($sortby == 2) {
    if ($order == 0) $ord = " ORDER BY vPhoneCode ASC"; else
        $ord = " ORDER BY vPhoneCode DESC";
}
if ($sortby == 3) {
    if ($order == 0) $ord = " ORDER BY eUnit ASC"; else
...
if ($keyword != '') {
    if ($option != '') {
        ...
        if ($eStatus != '') {
            $ssql .= " AND (vCountry LIKE '%" . $keyword . "%' OR vPhoneCode LIKE '%" . $keyword . "%' OR vCountryCodeISO3 LIKE '%" . $keyword . "%') AND eStatus = '" . clean($eStatus) . "'";
        } else {
            $ssql .= " AND (vCountry LIKE '%" . $keyword . "%' OR vPhoneCode LIKE '%" . $keyword . "%' OR vCountryCodeISO3 LIKE '%" . $keyword . "%')";
        }
    }
            ...
            <div class="table-list">
			  ...
			  <a href="javascript:void(0);" onClick="Redirect(1,<?php
					...
					<a href="javascript:void(0);" onClick="Redirect(3,<?php
                                        ...
                                        <?php if (strtoupper($RIDE_DRIVER_CALLING_METHOD) == "VOIP") { ?>
                                            ...
                                            <tr class="gradeA">
                                                ...
                                                <?php if (COUNTRY_IMAGE_UPLOAD == 'Yes') { ?>
                                                    <td style="text-align:center;"><?php if (!empty($data_drv[$i]['vSImage']) && file_exists($tconfig["tsite_upload_country_images_path"] . $data_drv[$i]['vSImage'])) { ?>
                                                        ...
                                                        <img src="<?= $tconfig["tsite_url"] . 'resizeImg.php?w=40&h=40&src=' . $tconfig["tsite_upload_country_images"] . $data_drv[$i]['vSImage'] ?>"
                                                             width="20" height="20">
                                                    <?php } else { ?>
                                                        <!-- <img src="<?= $tconfig["tsite_url"] . "webimages/icons/DefaultImg/in_s.png" ?>" width="20" height="20"> -->
                                                        <img src="<?= $tconfig['tsite_url'] . 'resizeImg.php?w=40&h=40&src=' . $tconfig['tsite_url'] . 'webimages/icons/DefaultImg/in_s.png' ?>"
                                                             width="20" height="20">
                                                    <?php } ?>
                                                        ...
                                                        <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iCountryId']; ?>">
                                                            <ul>
                                                                ...
                                                                <li class="entypo-twitter" data-network="twitter">
                                                                    <a href="country_action.php?id=<?= $data_drv[$i]['iCountryId']; ?>"
