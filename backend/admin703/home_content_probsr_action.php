...
if (!empty($img_arr)) {
    ...
    foreach ($img_arr as $key => $value) {
        ...
        if (!empty($value['name'])) {
            ...
            if ($key == 'secure_section_img' || $key == 'secure_section_img_first') $key = 'lSecuresafeSection';
...
if (isset($_POST['submit'])) {
    ...
    $check_file_query = "SELECT lHowitworkSection,lSecuresafeSection,lGeneralBannerSection,lCalculateSection,lRegisterSection,lSafeSection FROM $tbl_name where vCode='" . $vCode . "'";
    $check_file = $obj->MySQLSelect($check_file_query);
    ...
    $book_section_arr = !(empty($sectionData)) ? array_merge($sectionData, $book_section_arr) : $book_section_arr;
    for ($i = 1; $i <= 4; $i++) {
        $book_section_arr['bs_title' . $i . '_' . $vCode] = isset($_POST['book_section_bs_title' . $i]) ? $_POST['book_section_bs_title' . $i] : '';
        $book_section_arr['bs_desc' . $i . '_' . $vCode] = isset($_POST['book_section_bs_desc' . $i]) ? $_POST['book_section_bs_desc' . $i] : '';
    }
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-11">
                                                        <input type="text" class="form-control"
                                                               name="book_section_bs_title<?php echo $i; ?>"
                                                               id="book_section_bs_title<?php echo $i; ?>"
                                                               value="<?= $book_section['bs_title' . $i . '_' . $vCode]; ?>"
                                                               placeholder="Title">
                                                    </div>
                                                </div>
                                                <!-- Description  -->
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-11">
                                                        <textarea class="form-control"
                                                                  name="book_section_bs_desc<?php echo $i; ?>"
                                                                  id="book_section_bs_desc<?php echo $i; ?>"
                                                                  value="<?= $book_section['bs_desc' . $i . '_' . $vCode]; ?>"
                                                                  placeholder="Description"
                                                                  rows="3"><?= $book_section['bs_desc' . $i . '_' . $vCode]; ?></textarea>
                                                    </div>
                                                </div>
                                                <!-- Image  -->
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-11">
                                                        <?php if ($book_section['bs_img' . $i . '_' . $vCode] != '') { ?>
                                                            <img src="<?= $tconfig["tsite_upload_apptype_page_images"] . $template . '/' . $book_section['bs_img' . $i . '_' . $vCode]; ?>"
                                                                 class="innerbg_image"/ style="max-height:100px;">
                                                        <?php } ?>
                                                        <input type="file" class="form-control FilUploader"
                                                               name="book_section_bs_img<?php echo $i; ?>"
                                                               id="book_section_bs_img<?php echo $i; ?>"
